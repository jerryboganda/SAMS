...
import { Card, Button, Badge, Modal, Drawer } from "../../components/ui";
...
export const BookmarksPage: React.FC = () => {
  ...
  useEffect(() => {
    // Filter initial mock questions that are bookmarked
    const list = MOCK_QUESTIONS.filter((q) => q.isBookmarked);
    setBookmarkedList(list);
  }, []);

  // Open Drawer for a specific question
  ...
  return (
    <div className="space-y-8 pb-12 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        ...
        >
          Practice Bookmarks ({bookmarkedList.length})
        </Button>
      </div>
      ...
      {bookmarkedList.length === 0 ? (
        <Card className="p-12 text-center space-y-4 border-slate-200 bg-white rounded-2xl shadow-xs">
          ...
          <div className="space-y-1">
            <h3 className="text-base font-extrabold text-[#0E2A47]">No Bookmarked Questions Saved</h3>
                ...
                {bookmarkedList.map((q) => (
                  <tr key={q.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="p-4">
                      <Badge
                        variant={
                          q.difficulty === "easy" ? "emerald" : q.difficulty === "hard" ? "danger" : "warning"
                        }
                        size="sm"
                      >
      ...
      <Drawer
        ...
        title="Bookmarked Vignette Inspector"
        position="right"
      >
        {selectedQuestion && (
